#!/usr/bin/env python3
"""Plot NSKT snapshot comparisons for a selected Reynolds-number index."""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Plot NSKT snapshot comparisons for a selected Reynolds-number index."
    )
    parser.add_argument(
        "--re-id",
        type=int,
        default=9,
        help="Index into RE_EVAL_LIST. Examples: 5=12000, 7=24000, 9=36000; -1 cycles all eval Re values.",
    )
    ...
    parser.add_argument(
        "--force-refresh",
        action="store_true",
        help="Regenerate FLEX/UNet/SuperSloMo and EDEN caches before plotting.",
    )
    parser.add_argument(
        "--cache-dir",
        type=Path,
        default=REPO_ROOT / "analysis",
        help="Directory for comparison caches and output figures.",
    )
    parser.add_argument(
        "--list-re",
        action="store_true",
        help="Print available Reynolds-number indices and exit.",
    )
...
def apply_cli_overrides(config, args):
    if args.list_re:
        for idx, value in enumerate(RE_EVAL_LIST):
            print(f"{idx}: {value}")
        raise SystemExit(0)
    if args.re_id < -1 or args.re_id >= len(RE_EVAL_LIST):
        raise ValueError(f"--re-id must be -1 or in 0..{len(RE_EVAL_LIST) - 1}.")

    config["re_id"] = args.re_id
...
CONFIG["cache_re_label"] = cache_re_label
...
def build_dataset(config):
    ...
    # NSKT_eval uses random patch origins. The seed makes a fresh cache reproducible.
    dataset = NSKT_eval(
...
def visualize_comparison(result, config):
    ...
    save_path = config["cache_dir"] / f"nskt_{config['cache_re_label']}_sample_{result['sample_index']}_comparison.png"
