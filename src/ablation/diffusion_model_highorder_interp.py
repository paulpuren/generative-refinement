...
`x0` and `x1` are encoded by a shared spatial condition branch, while scalar
conditioning (`tau`, `dt`, `Re`) is embedded separately and routed through the
denoising backbone at every ResBlock.
"""
...
class DiffusionModel(nn.Module):
    ...
    def forward(
        ...
        est_snapshot, _ = self._estimate_snapshot(
            cond_snapshot_start,
            cond_snapshot_end,
            target_interp_step,
            total_interp_steps,
    ...
    def sample(
        ...
        est_snapshot = est_snapshot.to(device)
