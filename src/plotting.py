...
def plot_samples(
    ...
    for i in range(ncol): 
        axarr[0, i].imshow(cond_snapshot[i, 0, :, :], cmap = cmocean.cm.balance)
        axarr[0, i].set_xticks([])
        axarr[0, i].set_yticks([])
        axarr[0, i].title.set_text("x_t")  

    for i in range(ncol):
        error = (
            np.linalg.norm(samples[i, 0, :, :] - targets[i, 0, :, :]) /
            np.linalg.norm(targets[i, 0, :, :])
        )

        axarr[1, i].imshow(samples[i, 0, :, :], cmap = cmocean.cm.balance)
        axarr[1, i].set_xticks([])
        axarr[1, i].set_yticks([])
        axarr[1, i].title.set_text(f"Interpolated (RFNE={error:.3f})")  
...
#cond_snapshot, targets = next(iter(dataloader))
#samples = targets

#plot_samples(samples, cond_snapshot, targets, PATH='', epoch=1)
